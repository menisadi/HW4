 ...
 */
Link::Link(const std::string& data,  Link* link) : next_(link), data_(data)
{}

/** copy constructor
 *  This only copies the data of the link - no deep copy and no copy of next to avoid sharing Links between lists.
 */
Link::Link(const Link& aLink): next_(0), data_(aLink.getData())
{}
 ...
 */
List::List(const List &aList) : head_(aList.copy())
{}
...
List & List::operator=(const List &L)
{	
	Link *headOrig = head_; 	// remember original head
	head_ = L.copy();			// make head_ point to a copy of L		   	

	while (headOrig)  			// delete the original List
	{
	    Link* tmp = headOrig;
    	headOrig = headOrig->getNext();
    	delete tmp;
	}		     
